...
Because the setup provided in this repository is *secure by default*, the passwords for [built-in users][builtin-users]
**must be initialized** upon the first startup:

```console
$ docker-compose exec -T elasticsearch 'bin/elasticsearch-setup-passwords' auto --batch
```

Passwords for all 6 built-in users will be randomly generated. Take note of them and update the passwords for the
`kibana` and `logstash_system` users inside the Kibana and Logstash configuration files respectively, as well as inside
the Logstash default pipeline file. See the [Configuration](#configuration) section below.
